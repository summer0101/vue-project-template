<template>
  <div style="width:100%">
    <table width="100%">
      <tr>
        <td style="width:80px;" v-if="!!showRange">
          <el-select
            v-model="currentRangeTypeValue"
            @change="handleRangeOptionChange"
          >
            <el-option
  ...
  methods: {
    handleRangeOptionChange() {
      ...
      if (val == "0") {
        ...
        _toValue = this.$util.datetimeFormat(
          moment()
            .endOf("day")
            .toDate()
        );
      } else if (val == "1") {
        _fromValue = this.$util.datetimeFormat(
          moment()
            .startOf("day")
            .subtract(1, "days")
        );
